...
def main():
    ...
    parser.add_argument("--output", type=str, default="output.jpg", help="The output file name.")
    ...
    if photomosaic.get_available_tiles_count() < (main_image.width // args.tile_size) * (main_image.height // args.tile_size):
        print("Not enough tiles to create the photomosaic.")
        print(f"You are trying to generate a photomosaic that requires {(main_image.width // args.tile_size) * (main_image.height // args.tile_size)} tiles, but there are only {photomosaic.get_available_tiles_count()} tiles available.")
        print("Please generate more tiles for this project, or change the configuration to use less tiles.")
        exit()
